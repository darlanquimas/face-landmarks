export const TRIANGULATION: readonly number[] = [
...
const drawPath = (ctx: CanvasRenderingContext2D, points: Point2D[], closePath: boolean): void => {
  ...
  predictions.forEach((prediction) => {
    ...
    for (let i = 0; i < TRIANGULATION.length / 3; i++) {
      const points: Point2D[] = [
        ...
        TRIANGULATION[i * 3 + 1],
        TRIANGULATION[i * 3 + 2],
      ].map((index) => keypoints[index] as Point2D);
      drawPath(ctx, points, true);
    }

    // Draw Dots
    for (let i = 0; i < keypoints.length; i++) {
      const [x, y] = keypoints[i] as Point2D;
